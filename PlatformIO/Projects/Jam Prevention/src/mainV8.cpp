//First successfull implementation of acknowledge based transmission. Every time a node transmits (except in beacon mode!) a messages it expects an acknowledge. If the acknowledge isn't received in a given time frame the transmission will be repeated.
      ...
      if(duration <= WaitAckDuration)
      {
        ...
        {  
          ...
          if(messageID == AcknowledgeID)
          {
            ...
            else if(MeasureAck && (target_address == DeviceID) && (source_address == message_target))
            {
              MeasureAck = false;
              waitack_first = true;
              STATE = HoldTimeSlot;
