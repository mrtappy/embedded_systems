...
void ResetMeter();
void MotorControl(int PulseDetect, double distance);
...
void setup() 
{
  ...
  pinMode(53, OUTPUT); //Transmitter trigger
  digitalWrite(53, LOW);
  PWM_Setup(); 
  ...
  //Motor PWM
    //Ports
    PIOC -> PIO_PDR |= PIO_PDR_P2 | PIO_PDR_P4; //Enable peripheral control of PC21
    ...
    PIOC -> PIO_ABSR |= PIO_ABSR_P2 | PIO_ABSR_P4; //Select peripheral type B which is PWM Output
    ...
    REG_PWM_CDTY0 = PWM_CDTY_CDTY(175);   //default value
    ...
    REG_PWM_CDTY1 = PWM_CDTY_CDTY(175); //default value
...
void measure(int PulseDetect, int ReadChannel, int waiting)
{
  ...
  while(measure_duration < max_duration)
  {
    ...
    if(loops < maxloops )
    {
      ...
      if(first_run == 0 && distance >= 50)
      {
        MotorControl(PulseDetect, distance);
        first_run = 1;
      }
