...
#include <SystemStates.h>
//SystemStates library for UltraSonicXx9000 Turbo "System State Machine"

#include <MeasureStates.h>
extern long TransmitDuration;
extern long TransmitStartTime;
extern long MeasureStartTime;
extern long MeasureDuration;
extern long TimeToObstacle;
extern long ObstacleEvalStartTime;
extern long ObstacleEvalDuration;
//MeasureStates library for Measure State Machine

#include <RumbleStates.h>
//RumbleStates library for Rumble State Machine
...
//
#define AutCycleTime    10
void smSystemInit();      // forward reference to state smInit
void smCycle();     // forward reference to state smCycle
StateMachine SystemAutomat(smSystemInit, smCycle, AutCycleTime);

lcDateTime  dt;             // Time-Structure of LoopCheck

// basic definitions for the measure state machine
//
#define AutCycleTime    10
void smCycle();     // forward reference to state smCycle
StateMachine MeasureAutomat(smMeasureInit, smCycle, AutCycleTime);

// basic definitions for the rumble state machine
//
#define AutCycleTime    10
void smCycle();     // forward reference to state smCycle
StateMachine RumbleAutomat(smRumbleInit, smCycle, AutCycleTime);
...
int run {0};
double Air_Speed {0};
double EnviornmentTemperature {0};
bool STATE_CONT_MEASURE {false};
bool STATE_INTERVAL_MEASURE {false};
...
void loop()
{
  ...
  if(loopCheck.timerMicro(lcTimer0, AutCycleTime, 0)) // clock of state machine
  {
    if(SystemState)
    {
      SystemAutomat.run();      // switching to the next state (defined by passed state)  
    }
